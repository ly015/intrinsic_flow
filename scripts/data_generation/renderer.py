...
class SMPLRenderer(object):
    ...
    def __call__(self,
        ...
        imtmp, visibility_image = render_model(
            verts,
            self.faces,
            w,
            h,
        ...
        imtmp = (imtmp * 255).astype('uint8')
        if return_visibility_image:
            return imtmp, visibility_image
        else:
            return imtmp
...
def render_model(verts,
    ...
    imtmp, visibility = simple_renderer(rn, verts, faces, color=color)
